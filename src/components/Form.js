...
export default function Form(props) {
  ...
  return (
    ...
    <div className="mb-3">
      <textarea className="form-control" id="mybox" rows="8" onChange={handleOnChange} value={text} placeholder="Enter Text Here" style={{backgroundColor:props.mode==='dark'?'#2857a6':'white',color:props.mode==='dark'?'white':'#0f3056'}}></textarea>
      </div>
      ...
      <button className="btn btn-primary mx-1 my-1" onClick={handleDelclick}>Clear Text</button>
      <button className="btn btn-primary mx-1 my-1" onClick={handleExtraSpaces}>Clear Extra Spaces</button>
    </div>
    <div className="contianer my-3" style={{color:props.mode==='dark'?'white':'#0f3056'}}>
      <h2>Your Text Summary</h2>
      <p>{text.split(' ').filter((element)=>{return element.length!==''.length}).length} words and {text.length} characters have entered.</p>
      <p>Reading Speed = {0.008*text.split(" ").filter((element)=>{return element.length!==0}).length} wpm </p>
      <h3>Preview</h3>
